...
MAT_GUTTER = 0.052  # board between two hung prints
BEVEL = 0.0068  # depth of the 45 degree cut
...
def layout(ratios: Sequence[float], width: int, height: int) -> list[Box]:
    ...
    span = sum(widths) + gutter * (len(widths) - 1)
...
def render(
    ...
    *,
    ...
    depth = max(1, round(height * BEVEL))
...
def render_jpeg(
    ...
    *,
    ...
    buf = io.BytesIO()
    render(plates, size=size, light=light).save(
